...
export function HallOfFame() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setUserBest(best);
      setLoadedGameId(selectedGame.id);
    });
  ...
  return (
    <div className="page-enter max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="text-center mb-8 pt-4">
        <Trophy
          className="w-12 h-12 text-neon-yellow mx-auto mb-4"
          style={{ filter: "drop-shadow(0 0 15px #f5ff00)" }}
        />
        <h1 className="font-pixel text-xl sm:text-3xl text-neon-yellow animate-flicker mb-2">
          SALÓN DE LA FAMA
        </h1>
        <p className="font-mono text-xs text-gray-500 uppercase tracking-wide">
          Las mejores puntuaciones de todos los tiempos
        </p>
      </div>
      ...
      {user && userBest && (
        <div className="mb-6 bg-vault-panel border-2 border-neon-cyan/30 rounded-lg p-4 flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-pixel text-[10px] text-neon-cyan uppercase">
              TU MEJOR MARCA
            </span>
          </div>
      ...
      <div className="bg-vault-panel border-2 border-vault-border rounded-lg overflow-hidden">
        ...
        <div className="grid grid-cols-[60px_1fr_120px_100px] sm:grid-cols-[80px_1fr_160px_120px] gap-2 px-4 py-3 border-b border-vault-border bg-vault-bg/50">
          ...
          <span className="font-pixel text-[10px] text-gray-500 uppercase text-right">
            Puntuación
          </span>
          <span className="font-pixel text-[10px] text-gray-500 uppercase text-right hidden sm:block">
            Fecha
          </span>
        </div>
        ...
        {loading ? (
          <div className="p-12 text-center">
            <p className="font-mono text-xs text-gray-500">CARGANDO...</p>
          </div>
        ) : scores.length === 0 ? (
          <div className="p-12 text-center">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`font-pixel text-[10px] ${medal.color}`}>
                      {i + 1}
                    </span>
                  </div>
                  <span
                    className={`font-mono text-sm truncate ${isUser ? "text-neon-cyan" : "text-gray-300"}`}
                  >
                    {row.player_name}
                    {isUser && (
                      <span className="ml-2 font-pixel text-[8px] text-neon-cyan">
                        TÚ
                      </span>
                    )}
                  </span>
                  <span
                    className={`font-pixel text-xs text-right ${i < 3 ? medal.color : "text-white"}`}
                  >
