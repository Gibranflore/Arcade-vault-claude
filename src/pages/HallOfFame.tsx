...
import { fetchLeaderboard, fetchUserBest } from '../lib/scores';
...
export function HallOfFame() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchLeaderboard(selectedGame.id, 10).then((rows) => {
      setScores(rows);
      setLoading(false);
    });
    if (user) {
      fetchUserBest(selectedGame.id, user.id).then(setUserBest);
    } else {
      setUserBest(null);
    }
  }, [selectedGame, user]);
      ...
      {user && userBest && (
        <div className="mb-6 bg-vault-panel border-2 border-neon-cyan/30 rounded-lg p-4 flex items-center gap-4">
          ...
          <div className="flex-1 flex items-center justify-end gap-4">
            <span className="font-mono text-sm text-gray-300">{user.user_metadata?.username ?? 'Tú'}</span>
        ...
        {loading ? (
          <div className="p-8 space-y-2">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="h-12 bg-vault-bg rounded animate-pulse" />
            ))}
          </div>
        ) : scores.length === 0 ? (
